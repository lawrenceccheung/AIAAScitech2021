...
def calc_average_profile_last_hour():
  a = Dataset(sorted(glob.glob('post_processing/abl_statistics120000.nc'))[-1])
  tstart_idx = np.searchsorted(a.variables['time'][:],15000.0)
  tend_idx = np.searchsorted(a.variables['time'][:],20000.0)

  print(tstart_idx)
  mean_profiles = a.groups['mean_profiles']
  vars = list(mean_profiles.variables)[1:]
  b = pd.DataFrame({ v: np.average(mean_profiles[v][tstart_idx:tend_idx,:],axis=0) for v in vars})
  b['z'] = mean_profiles['h'][:]
...
def calc_average_istats_last_hour():
  ...
  tstart_idx = np.searchsorted(a.variables['time'][:],15000.0)
  tend_idx = np.searchsorted(a.variables['time'][:],20000.0)
  i_stats = { v: float(np.average( a.variables[v][tstart_idx:tend_idx])) for v in vars}
...
def calc_average_spectra_last_hour():
  ...
  start_idx = np.searchsorted(tttime, 15000)
